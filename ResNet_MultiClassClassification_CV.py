...
# and also the training, validation, and test sets sizes, respectively.
batch_size = 16
#train_size = 10_000
#val_size = 2_000
train_size = 12800
...
# We download the cifar10 training dataset if it does not exist at the location specified by the given path
...
# We download the cifar10 test dataset if it does not exist at the location specified by the given path
...
class ResNetBlock(nn.Module):
    ...
    def __init__(self, in_channels: int, out_channels: int, id: bool = True):
        ...
        self.convolutionalLayer1 = self.conv(in_channels=in_channels, out_channels=out_channels, kernel_size=3, stride=1)
        self.batchNormLayer1 = nn.BatchNorm2d(num_features=out_channels)

        self.convolutionalLayer2 = self.conv(in_channels=out_channels, out_channels=out_channels, kernel_size=3, stride=1)
...
class ResNet_MultiClassClassification(nn.Module):
...
def train(model, train_loader, val_loader, test_loader, num_epochs, criterion, optimizer):
    ...
    # Save the best model on local disk, based on the Accuracy of the Vadidation Set
...
if __name__ == '__main__':
    num_epochs = 10
    resnet = ResNet_MultiClassClassification()
